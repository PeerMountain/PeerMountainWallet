...
class Connection {
  ...
  final StreamController<AbstractStanza?> _inStanzaStreamController = StreamController.broadcast();

  final StreamController<AbstractStanza> _outStanzaStreamController = StreamController.broadcast();

  final StreamController<Nonza> _inNonzaStreamController = StreamController.broadcast();

  final StreamController<Nonza> _outNonzaStreamController = StreamController.broadcast();

  final StreamController<XmppConnectionState> _connectionStateStreamController = StreamController.broadcast();
...
""";
  ...
  String prepareStreamResponse(String response) {
    ...
    if (response1.contains('stream:stream') && !(response1.contains('</stream:stream>'))) {
  ...
  Future<void> openSocket() async {
    ...
    try {
      ...
      return await socket
          .connect(account.host ?? account.domain, account.port, map: prepareStreamResponse)
          .then((socket) {
  ...
  void handleResponse(String response) {
    ...
    if (fullResponse.isNotEmpty) {
      ...
      try {
        ...
        _unparsedXmlResponse +=
            fullResponse.substring(0, fullResponse.length - 13); //remove  xmpp_stone end tag
...
//      });
      ...
      xmlResponse.descendants
          ...
          .forEach((feature) => connectionNegotatiorManager.negotiateFeatureList(feature));
  ...
  void _processState(XmppConnectionState state) {
    if (state == XmppConnectionState.Authenticated) {
      authenticated = true;
      _openStream();
    } else if (state == XmppConnectionState.Closed || state == XmppConnectionState.ForcefullyClosed) {
      authenticated = false;
    }
  }
  ...
  void startSecureSocket() {
    ...
    _socket!.secure(onBadCertificate: _validateBadCertificate).then((secureSocket) {
      if (secureSocket == null) return;

      secureSocket.cast<List<int>>().transform(utf8.decoder).map(prepareStreamResponse).listen(handleResponse,
          onError: (error) => {handleSecuredConnectionError(error.toString())},
  ...
  bool elementHasAttribute(xml.XmlElement element, xml.XmlAttribute attribute) {
    var list = element.attributes
        .firstWhereOrNull((attr) => attr.name.local == attribute.name.local && attr.value == attribute.value);
    return list != null;
  }
