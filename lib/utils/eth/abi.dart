...
Uint8List encodeSingle(String type, dynamic arg) {
  ...
  if (type == 'address') {
    ...
    if (num.bitLength > size) {
      throw ArgumentError('Supplied uint exceeds width: $size vs ${num.bitLength}');
    }
    ...
    var num = parseNumber(arg);
    if (num.bitLength > size) {
      throw ArgumentError('Supplied int exceeds width: $size vs ${num.bitLength}');
    }
...
List<int> parseTypeNxM(String type) {
  ...
  return [int.parse(tmp.group(1)!, radix: 10), int.parse(tmp.group(2)!, radix: 10)];
}
