...
import 'bigint.dart';
import 'utils.dart' show intToBuffer, isHexString, padToEven, stripHexPrefix;
...
int safeParseInt(String v, [int? base]) {
  if (v.startsWith('00')) {
    throw const FormatException('invalid RLP: extra zeros');
  }
...
Decoded _decode(Uint8List input) {
  ...
  if (firstByte <= 0x7f) {
    ...
    if (length == 2 && data[0] < 0x80) {
      throw const FormatException('invalid rlp encoding: byte must be less 0x80');
    }
    ...
    if (totalLength > input.length) {
      throw const FormatException('invalid rlp: total length is larger than the data');
    }
