...
class Utils {
  ...
  static bool isYesterday(String inputtedDate, String format) {
    try {
      var inputFormat = DateFormat(format);
      var date1 = inputFormat.parse(inputtedDate);
      final now = DateTime.now().subtract(const Duration(days: 1));
      return now.day == date1.day;
    } catch (e) {
      showLog("isYesterday exception =====>>> $e");
    }

    return false;
  }

  static bool isCurrentDate(String inputtedDate, String format) {
    try {
      var inputFormat = DateFormat(format);
      var date1 = inputFormat.parse(inputtedDate);
      return DateTime.now().day == date1.day;
    } catch (e) {
      showLog("isCurrentDate exception =====>>> $e");
    }

    return false;
  }

  static String formatMillis(dynamic millis, {String format = Constants.longDateFormat}) {
    try {
      final date1 = DateTime.fromMillisecondsSinceEpoch(millis.toString().toInt());
  ...
  static openEmail(String email) {
    final Uri emailLaunchUri = Uri(
      scheme: 'mailto',
      path: email,
      query: Utils.encodeQueryParameters(
        <String, String>{
  ...
  static void openWhatsapp() async {
    try {
      ...
      const supportText = "I would like to get support for below concerns :- \n\n";

      if (Platform.isIOS) {
        final iOSUrl = "https://wa.me/$whatsapp?text=${Uri.encodeComponent(supportText)}";
  ...
  Future<Uri> shortenUri(Uri uri, String bitlyToken) async {
    ...
    final responseJson = json.decode(responseBody) as DMap;
  ...
  static Future<DMap?> pickImage(ImageSource source) async {
    try {
      final pickedFile = await Utils.pickImageFromSource(source: source);
      ...
      if (pickedFile != null) {
        ...
        if (source == ImageSource.camera) {
          ...
          fileName = pickedFile.path.substring(pickedFile.path.lastIndexOf("/")).substring(1);
        }
        return {
          'file': pickedFile,
          'file_name': fileName,
  ...
  static Future<File?> pickImageFromSource({required ImageSource source}) async {
  ...
  static Future<File?> compressImage(XFile? imageToCompress) async {
    ...
    File? croppedFile = await ImageCropper.cropImage(
      ...
      androidUiSettings: const AndroidUiSettings(
          ...
          toolbarColor: Palette.accentColor,
    ...
    im.Image? image = im.decodeImage(croppedFile.readAsBytesSync());
    if (image == null) {
      return croppedFile;
    }
    ...
    final kFile = File('$path/temp_image_$rand.jpg')..writeAsBytesSync(im.encodeJpg(image, quality: 85));

    final targetPath = dir.absolute.path + "/temp_${now.millisecondsSinceEpoch}.jpg";
    final compressedFile = await compressAndGetFile(kFile, targetPath);
  ...
  static List<ChatModel> getRandomMessages(int length) {
    ...
    for (var i = 0; i < length; ++i) {
      final address = "0x${randomString(40)}";
      final randomInt = Random().nextInt(2);
      final randomDayInt = Random().nextInt(30);

      final createdOn = DateTime(now.year, now.month, randomDayInt);

      final o = ChatModel(
        senderJid: randomInt == 1 ? cryptoAccount.address!.addXmppKycHost() : address.addXmppKycHost(),
        receiverJid: randomInt == 1 ? address.addXmppKycHost() : cryptoAccount.address!.addXmppKycHost(),
        message: randomString(Random().nextInt(100)),
  ...
  static Future<String> loadAsset({required String filename}) async {
